function varargout=sigstar(groups,stats,nosort)
    ...
    % edits to include multcompare output as stats - SB, 2021
    if istable(stats)
        conditions=unique(stats.cond_1);
        groups1=[]; 
        for ii=1:numel(conditions)
            for jj=ii+1:numel(conditions)
                idx=stats.cond_1==conditions(ii) & stats.cond_2==conditions(jj);
                if stats.pValue(idx) <= 0.05
                    groups1=[ii,jj, stats.pValue(idx); groups1];
                end
            end
        end
        if ~isempty(groups1)
