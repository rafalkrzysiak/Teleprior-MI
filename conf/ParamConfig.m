...
agents = 1; % -- number of agents in the simulation including the human robot
param.r_visible = 1.33*ones(1,agents); % -- visible range of the robots (m), human robot has slightly higher range % 4 -> 2
param.r_FOV = 2*pi*ones(1,agents);
param.r_FOV(1) = 78*pi/180; % -- human robot 70*pi/180 has FOV of 70 degrees, autonomous robots have 360 degrees
...
param.w = diag([.1, .1, .01, .05, .05]); % -- disturbance
