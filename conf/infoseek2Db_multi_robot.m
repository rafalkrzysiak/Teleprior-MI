function infoseek2Db_multi_robot(param, img, saveIn, maps, ...
    saveFrames, target_loc, exp_id, condition, ConfigSetup)
                ...
                if abs(Z(:,k,1,robot)) < param.r_FOV(robot)/2
                    ...
                    if rij(robot,1) < param.r_visible(robot) % -- must be closer to get a diffinitive answer
                        ...
                        for rob = 1:param.agents
                            Xs(:,k+1:kF,1,rob) = Xs(:,k,1,rob).*ones(5,kF-k);
                            Xh(:,k:kF,1,rob) = Xh(:,k-1,1,rob).*ones(5,kF-(k-1));
                        end
...
%             pts = find(rb_info(:,2) < pi/6 & rb_info(:,2) > -pi/6 & rb_info(:,1) < param.r_visible(robot));
            pts = find(rb_info(:,2) < param.r_FOV(robot)/2 & ...
                       rb_info(:,2) > -param.r_FOV(robot)/2 & ...
                       rb_info(:,1) < param.r_visible(robot));
...
for kk = 1:np
    ...
    if (X_target > 0 && Y_target > 0 && X_target < X_limit && Y_target < Y_limit)
        ...
        if param.config == "yMxT" || param.config == "yMyT"
